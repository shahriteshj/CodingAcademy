...
public class HQLPaging {
	...
	public static void main(String[] args) {
		...
		try {
			...
			//before listing the query .... we need to set the 
			//setFirstResult(1) and setMaxResults()
			//by default setFirstResult is 0 row
			//row index always start from 0 
			//query.setFirstResult(1);//2nd row
			query.setMaxResults(6);//max only 6 rows
			List<Customer> results =query.list();
			for (Customer c1 : results) {
				System.out.println(c1);
			}
			//List<String> res=query.list() ;//= query.list();
			/*for (String c1 : res) {
				System.out.println(c1);
			}*/
